	...
	var PokerHand = function (hand) {
		...
		this.setHandRank = function () {
			...
			if (strtFlg || flushFlg) {
				if (strtFlg && flushFlg)
					if (ranks[4] == 10)
						_handRank[5] = ROYALFLUSH;
					else
						_handRank[5] = STRT_FLUSH;
				else if (strtFlg)
					_handRank[5] = STRAIGHT;
				else
					_handRank[5] = FLUSH;
			} else {
				if (this.isFour(ranks))
					_handRank[5] = FOUR_KIND;
				else if (this.isFull(ranks))
					_handRank[5] = FULL_HOUSE;
			}
			...
			if (this.isThree(ranks))
				_handRank[5] = THREE_KIND;
			else if (this.isTwoPair(ranks))
				_handRank[5] = TWO_PAIR;
		...
		this.convertHandRank = function() {
			...
			retVal += handRank[5] * Math.pow(16.0, 5);
