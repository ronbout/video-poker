...
(function ($) {
	...
	var Vp = function () {
		...
		this.dealHand = function () {
			...
			_curHand.forEach(function (card, i) {
				$('#card' + (i + 1)).prop('src', Deck.getCardImage(card)).prop('alt', Deck.getCardDesc(card)).prop('title', Deck.getCardDesc(card));
			});
		...
		this.draw = function () {
			_holdCards.forEach(function (holdCard, i) {
				if (!holdCard) {
					...
					$('#card' + (i + 1)).prop('src', cardImage).prop('alt', Deck.getCardDesc(card)).prop('title', Deck.getCardDesc(card));
				}
			});
			// determine hand type
		...
		this.calcPayout = function (handRank) {
			...
			switch (handRank[5]) {
				...
				case PAIR:
					...
					payout = payStructure[handRank[5]][Deck.getRank(handRank[4]) - 2] * _curBet;
			}
			return payout;
		}
	...
	$(document).ready(function () {
		...
		var msgs = {
			...
			lost: 'Sorry. You lost.',
			won: 'You won $%%_amt_%%!',
			odds: 'The Calc Odds/EV button looks at every combination of hands \n' +
		...
		msgModal.html(msgs.odds.replace(/\nNote/g, '<br><br>Note').replace(/\n/g, ''));
